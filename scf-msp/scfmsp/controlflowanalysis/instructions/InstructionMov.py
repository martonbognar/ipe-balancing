...
class InstructionMov(AbstractInstructionTwoRegisters):
    ...
    def get_successors(self):
        # hardcoding representative paths for indirect jumps for switch8/16
        if (self.function.name.startswith("switch8_balanced") or self.function.name.startswith("switch16_balanced")) and self.arguments == ('r12', 'r0'):
            return [self.get_execution_point().forward(24), self.get_execution_point().forward(40)]
